...
- [ ] `/vote` - 참여코드 입력 페이지
  - [ ] 참여코드 입력 폼
  - [ ] 코드 유효성 검사
  - [ ] 에러 처리 (잘못된 코드, 이미 사용된 코드)

- [ ] `/vote/[code]` - 투표 진행 페이지
  - [ ] 참여코드로 접근 가능한 투표 목록 조회
  - [ ] 투표 선택
  - [ ] 후보자 목록 표시
  - [ ] 후보자 선택 UI
    - [ ] 단일 선택 (max_selections = 1)
    - [ ] 복수 선택 (max_selections > 1)
    - [ ] 선택 개수 제한 표시
  - [ ] 투표 제출
  - [ ] 중복 투표 방지
  - [ ] 투표 완료 후 코드 사용 처리

- [ ] `/vote/complete` - 투표 완료 페이지
  - [ ] 완료 메시지
  - [ ] 투표 요약 정보
...
### 4. 실시간 투표 현황
- [ ] `/admin/elections/[id]/monitor` - 실시간 모니터링
  - [ ] 투표율 표시 (사용된 코드 / 전체 코드)
  - [ ] 실시간 득표 차트 (recharts)
    - [ ] 막대 그래프
    - [ ] 파이 차트
...
### 5. 투표 결과
- [ ] `/admin/elections/[id]/results` - 결과 페이지
  - [ ] 최종 득표 결과
  - [ ] 당선자 표시
  - [ ] 투표율 통계
  - [ ] 마을별 투표율 (총대 선출인 경우)
...
**완료율**: 약 45% (기본 인프라, 관리자 기능, 투표 생성 관리)

**최근 완료**: Phase 1.1 - 투표 생성 및 관리 페이지 ✅

**다음 마일스톤**: Phase 1 완료 (핵심 투표 기능) → 70% 완료 예상

---

**마지막 업데이트**: 2025-11-05 - 투표 생성/목록/상세 페이지 완료
